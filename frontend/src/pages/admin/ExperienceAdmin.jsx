...
const EMPTY_FORM = {
  ...
  company: "", employment_type: "full_time", location: "",
...
export default function ExperienceAdmin() {
  ...
  const load     = () => api.get("/api/admin-control/experience/").then((r) => setData(Array.isArray(r.data) ? r.data : [])).catch(console.error);
  ...
  const open = (item = null) => {
    ...
    if (item) {
      setForm({
        ...
        achievements_uz:     item.achievements_uz     || "",
        achievements_ru:     item.achievements_ru     || "",
        achievements_en:     item.achievements_en     || "",
        responsibilities_uz: item.responsibilities_uz || "",
        responsibilities_ru: item.responsibilities_ru || "",
        responsibilities_en: item.responsibilities_en || "",
        teaching_focus_uz:   item.teaching_focus_uz   || "",
        teaching_focus_ru:   item.teaching_focus_ru   || "",
        teaching_focus_en:   item.teaching_focus_en   || "",
  ...
  const save = async () => {
    ...
    if (!form.title_uz){ setError("// Lavozim (UZ) kiritilmagan!"); return; }
    setSaving(true);
    try {
      const payload = {
        ...form,
    ...
    <>
      ...
      <div className="ex-wrap">
        <div className="ex-topbar">
          <div><div className="ex-title">Experience</div><div className="ex-sub">// ish tajribasi</div></div>
          <button className="ex-btn-primary" onClick={() => open()}><Icon name="plus" /> Yangi qo'shish</button>
        </div>
          ...
          <div key={item.id} className="ex-card">
            <div className="ex-body">
              ...
              <div className="ex-meta">
                ...
                <span className={`ex-tag ${TYPE_COLORS[item.employment_type] || "ex-tag-blue"}`}>{TYPE_LABELS[item.employment_type]}</span>
                {item.location && <span className="ex-tag ex-tag-purple">{item.location}</span>}
                <span className="ex-tag ex-tag-yellow">{item.start_date?.slice(0,7)} — {item.is_current ? "Hozirgacha" : item.end_date?.slice(0,7) || "—"}</span>
              </div>
              {item.responsibilities_uz && <div className="ex-text">{item.responsibilities_uz}</div>}
            </div>
            <div className="ex-actions">
              <button className="ex-btn-icon" onClick={() => open(item)}><Icon name="edit" /></button>
              <button className="ex-btn-icon danger" onClick={() => del(item.id)}><Icon name="trash" /></button>
            </div>
          </div>
        ))}
          ...
          <div className="ex-overlay" onClick={(e) => e.target === e.currentTarget && close()}>
            <div className="ex-modal">
              ...
              <div className="ex-modal-body">
                ...
                {/* LAVOZIM */}
                <div className="ex-field">
                  <label className="ex-label">LAVOZIM * ({formLang.toUpperCase()})</label>
                  {formLang === "uz" && <input className="ex-input" placeholder="Backend Developer" value={form.title_uz} onChange={f("title_uz")} />}
                  {formLang === "ru" && <input className="ex-input" placeholder="Бэкенд разработчик" value={form.title_ru} onChange={f("title_ru")} />}
                  {formLang === "en" && <input className="ex-input" placeholder="Backend Developer"  value={form.title_en} onChange={f("title_en")} />}
                </div>
                ...
                {/* VAZIFALAR */}
                <div className="ex-field">
                  <label className="ex-label">ASOSIY VAZIFALAR ({formLang.toUpperCase()})</label>
                  {formLang === "uz" && <textarea className="ex-textarea" placeholder="API yaratish..." value={form.responsibilities_uz} onChange={f("responsibilities_uz")} />}
                  {formLang === "ru" && <textarea className="ex-textarea" placeholder="Создание API..." value={form.responsibilities_ru} onChange={f("responsibilities_ru")} />}
                  {formLang === "en" && <textarea className="ex-textarea" placeholder="Building APIs..." value={form.responsibilities_en} onChange={f("responsibilities_en")} />}
                </div>

                {/* YUTUQLAR */}
                <div className="ex-field">
                  <label className="ex-label">YUTUQLAR ({formLang.toUpperCase()})</label>
                  {formLang === "uz" && <textarea className="ex-textarea" placeholder="Loyiha tezligini oshirdim..." value={form.achievements_uz} onChange={f("achievements_uz")} />}
                  {formLang === "ru" && <textarea className="ex-textarea" placeholder="Ускорил проект..." value={form.achievements_ru} onChange={f("achievements_ru")} />}
                  {formLang === "en" && <textarea className="ex-textarea" placeholder="Improved project speed..." value={form.achievements_en} onChange={f("achievements_en")} />}
                </div>

                {/* O'QITISH */}
                <div className="ex-row">
                  <div className="ex-field">
                    <label className="ex-label">O'QITISH ({formLang.toUpperCase()})</label>
                    {formLang === "uz" && <input className="ex-input" placeholder="Python, Backend" value={form.teaching_focus_uz} onChange={f("teaching_focus_uz")} />}
                    {formLang === "ru" && <input className="ex-input" placeholder="Python, Бэкенд"  value={form.teaching_focus_ru} onChange={f("teaching_focus_ru")} />}
                    {formLang === "en" && <input className="ex-input" placeholder="Python, Backend" value={form.teaching_focus_en} onChange={f("teaching_focus_en")} />}
                  </div>
                  <div className="ex-field">
              ...
              <div className="ex-modal-foot">
                ...
                <button className="ex-btn-save" onClick={save} disabled={saving || !form.title_uz || !form.dev}>
